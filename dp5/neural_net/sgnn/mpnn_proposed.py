...
class nmr_mpnn_PROPOSED(nn.Module):
    ...
    def __init__(self, 
        ...
        output_dim = len(quantiles) if quantiles is not None else 1
        self.readout_n[-1] = nn.Linear(pred_hid_feats, output_dim)
        self.readout_n_naive[-1] = nn.Linear(pred_hid_feats, output_dim)


    def forward(self, g, n_nodes, masks, targets=None):
...
class QuantileLoss(nn.Module):
    def __init__(self, quantiles, delta=1e-4):
        super().__init__()
        self.quantiles = torch.tensor(quantiles, dtype=torch.float32).reshape(1, -1).to('cuda')
        self.delta = delta

    def forward(self, pred, y):
        # Ensure y has the same number of dimensions as pred
        while y.dim() < pred.dim():
            y = y.unsqueeze(-1)
        
        # Broadcast y to match pred's shape
        y = y.expand_as(pred)
        ...
        # Huber loss
        abs_diff = diff.abs()
        quadratic = torch.min(abs_diff, torch.full_like(abs_diff, self.delta))
        linear = abs_diff - quadratic
        huber_loss = 0.5 * quadratic.pow(2) / self.delta + linear * self.delta
        
        # Quantile loss
        quantile_loss = torch.max((self.quantiles - 1) * diff, self.quantiles * diff)
        
        loss = (huber_loss + quantile_loss).mean(1)
        
        # Order loss
        q_order_loss = torch.clamp(pred[:, :-1] - pred[:, 1:] + 1e-6, min=0.0).sum(-1)
        
        return loss.mean() + q_order_loss.mean()
