...
class GraphDataset():
    ...
    def __init__(self, target, graph_representation, data_path=None, data_filename=None, mol_dict=None):
        ...
        if mol_dict is not None:
            self.load_from_dict(mol_dict)
        else:
            self.load()


    def load(self):
        [mol_dict] = np.load(os.path.join(self.data_path, self.data_filename), allow_pickle=True)['data']
        self.load_from_dict(mol_dict)

    def load_from_dict(self, mol_dict):
